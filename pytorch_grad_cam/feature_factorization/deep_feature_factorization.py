...
def dff(activations: np.ndarray, n_components: int=5):
    ...
    reshaped_activations = reshaped_activations - reshaped_activations.min(axis=0)        
...
class DeepFeatureFactorization:
    ...
    def __call__(self,
        ...
        if self.computation_on_concepts:
            with torch.no_grad():
                concept_tensors = torch.from_numpy(np.float32(concepts).transpose((1, 0)))
                concept_outputs = self.computation_on_concepts(concept_tensors).cpu().numpy()
